...
// ================== Finnhub: /stock/earnings (estructurado) ==================
// Sprint P3.1B.1 (Materiality Real-World Validation). Forma real de
// Finnhub /stock/earnings (a verificar contra la respuesta real del
// endpoint en el benchmark en vivo -- si el shape difiere, este
// normalizador se corrige antes de reportar cualquier resultado, nunca
// se fuerza un campo que no vino): { symbol, period, year, quarter,
// actual, estimate, surprise, surprisePercent }. A diferencia de FMP
// /earnings, Finnhub NO trae revenueActual/revenueEstimated en este
// endpoint -- esos quedan UNKNOWN honestamente, nunca inventados.
//
// Finnhub tampoco distingue una fecha de "reporte" separada de `period`
// (fin del trimestre fiscal) en este endpoint -- se usa `period` como
// occurred_at/published_at con esa limitacion explicita, en vez de
// fingir que existe un campo de fecha de anuncio real que no viene.
export function normalizeFinnhubEarnings(raw, { asset_id, ticker }) {
  const facts = {
    eps_actual: typeof raw.actual === "number" ? raw.actual : "UNKNOWN",
    eps_estimated: typeof raw.estimate === "number" ? raw.estimate : "UNKNOWN",
    revenue_actual: "UNKNOWN", // no disponible en este endpoint de Finnhub
    revenue_estimated: "UNKNOWN",
    earnings_date: raw.period || "UNKNOWN",
    fiscal_quarter: typeof raw.quarter === "number" ? raw.quarter : "UNKNOWN",
    fiscal_year: typeof raw.year === "number" ? raw.year : "UNKNOWN",
  };
  return {
    asset_id, ticker,
    event_type: EVENT_TYPE.EARNINGS,
    classification_method: CLASSIFICATION_METHOD.STRUCTURED_FIELD,
    headline: `Earnings ${ticker} (${raw.period || "?"}): EPS ${facts.eps_actual !== "UNKNOWN" ? facts.eps_actual : "?"} vs. est. ${facts.eps_estimated !== "UNKNOWN" ? facts.eps_estimated : "?"}`,
    occurred_at: raw.period || null,
    published_at: raw.period || null,
    facts,
    known_fact_count: countKnown(facts, expectedKeysFor(EVENT_TYPE.EARNINGS)),
    expected_fact_keys: expectedKeysFor(EVENT_TYPE.EARNINGS),
    requires_review: false,
    provider: "finnhub",
    source_url: null, // Finnhub earnings no trae url de nota -- el provenance es el endpoint mismo
  };
}
