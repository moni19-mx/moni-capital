...
export function buildNormalizedExtraction({ raw, assetResolution, accountMatch }) {
  ...
  // Se conserva la confidence del campo RAW junto con la fecha parseada
  // -- necesaria para decidir despues (en confirm, PENDING->EXECUTED) si
  // una nueva fecha tiene evidencia mas fuerte que la original.
  const transaction_date_confidence = dateParse.status === "PARSED" ? (raw.transaction.transaction_date?.confidence ?? null) : null;
  ...
  return {
    asset: {
      ...
      // name_raw: SOLO para mostrar en UI (ej. boton "Crear activo nuevo"
      // con el nombre ya sugerido) -- nunca se usa como fuente de verdad
      // de identidad, eso sigue siendo ticker_raw/asset_id.
      name_raw: raw.transaction.asset_name?.value ?? null,
    },
    account: {
