...
// ==================================================
// getAccountEquity -- CONTRATO FINAL (reemplaza la version anterior,
// que sumaba available_balance_value incorrectamente -- verificado
// contra datos reales de Binance USD-M, ver hallazgo de diseno).
// ==================================================
// accountContext: { accountType, provider, productType }
//
// Registry explicito -- vacio salvo entradas verificadas. Como con
// unitSemanticsRegistry para COIN-M, una entrada aqui es una afirmacion
// verificada de que "este campo reportado por este proveedor para este
// producto es equivalente a account equity" -- nunca una suposicion.
export const EQUITY_SOURCE_REGISTRY = {
  "BINANCE::USD_M": {
    equivalentField: "margin_balance_value",
    derivedFormula: "WALLET_PLUS_UNREALIZED_PNL",
  },
  // COIN-M: MISMA formula, denominada en el activo subyacente (BTC) en
  // vez de USDT -- WALLET_PLUS_UNREALIZED_PNL no asume ninguna moneda
  // especifica, opera sobre wallet_balance_value + unrealized_pnl_value
  // tal cual vengan, sin importar en que asset esten expresados. El
  // resultado sigue en BTC -- nunca se convierte a USD aqui.
  "BINANCE::COIN_M": {
    equivalentField: "margin_balance_value",
    derivedFormula: "WALLET_PLUS_UNREALIZED_PNL",
  },
...
export const ACCOUNT_EQUITY_RECONCILIATION_TOLERANCE_PCT = 2;

function computeDerivedEquityValue(balanceRow, formula) {
  if (formula === "WALLET_PLUS_UNREALIZED_PNL") {
    if (balanceRow.wallet_balance_value == null || balanceRow.unrealized_pnl_value == null) return null;
    return balanceRow.wallet_balance_value + balanceRow.unrealized_pnl_value;
  }
  return null;
}

// Diferencia porcentual segura contra reported=0 (evita division por
// cero): reported=0 y derived=0 -> 0% (consistentes); reported=0 y
// derived!=0 -> 100% (maximamente inconsistentes, fuerza REQUIRES_REVIEW).
function equityDiffPctSafe(reported, derived) {
  if (reported === 0) return derived === 0 ? 0 : 100;
  return (Math.abs(reported - derived) / Math.abs(reported)) * 100;
}

export function getAccountEquity(balanceRow, accountContext) {
  if (!balanceRow || typeof balanceRow !== "object") {
    throw new Error("getAccountEquity: se requiere un objeto balanceRow");
  }
  const { accountType, provider, productType } = accountContext || {};

  // Regla estructural para spot: no es un hecho de proveedor que haya
  // que verificar (a diferencia de futuros) -- una cuenta spot no tiene
  // apalancamiento ni PnL no realizado a nivel de cuenta, asi que su
  // equity es, por definicion, el wallet balance. Por eso vive aqui
  // como regla aparte, no como entrada de EQUITY_SOURCE_REGISTRY.
  if (accountType === "SPOT" && balanceRow.equity_value == null) {
    if (balanceRow.wallet_balance_value != null) {
      return {
        status: "OK", value: balanceRow.wallet_balance_value, provenance: "DERIVED",
        reported_candidate: null, derived_candidate: balanceRow.wallet_balance_value, warnings: [],
      };
    }
    return { status: "DATA_UNAVAILABLE", value: null, provenance: null, reported_candidate: null, derived_candidate: null, warnings: [] };
  }

  const registryKey = provider != null && productType != null ? `${provider}::${productType}` : null;
  const registryEntry = registryKey ? EQUITY_SOURCE_REGISTRY[registryKey] : null;

  // Precedencia A: equity_value explicito del proveedor.
  // Precedencia B: campo equivalente verificado, SOLO si esta en el registry.
  // available_balance_value NUNCA participa aqui ni en ningun otro punto.
  let reportedValue = null;
  let reportedProvenance = null;
  if (balanceRow.equity_value != null) {
    reportedValue = balanceRow.equity_value;
    reportedProvenance = "REPORTED_EQUITY";
  } else if (registryEntry && balanceRow[registryEntry.equivalentField] != null) {
    reportedValue = balanceRow[registryEntry.equivalentField];
    reportedProvenance = "REPORTED_EQUIVALENT_FIELD";
  }

  // Precedencia C: derivado, solo si el registry define formula para
  // este provider/product Y todos sus insumos existen.
  let derivedValue = null;
  if (registryEntry && registryEntry.derivedFormula) {
    derivedValue = computeDerivedEquityValue(balanceRow, registryEntry.derivedFormula);
  }

  if (reportedValue != null) {
    if (derivedValue != null) {
      const diffPct = equityDiffPctSafe(reportedValue, derivedValue);
      if (diffPct > ACCOUNT_EQUITY_RECONCILIATION_TOLERANCE_PCT) {
        return {
          status: "REQUIRES_REVIEW", value: null, provenance: null,
          reported_candidate: reportedValue, derived_candidate: derivedValue,
          warnings: ["ACCOUNT_EQUITY_RECONCILIATION_MISMATCH"],
        };
      }
      return {
        status: "OK", value: reportedValue, provenance: reportedProvenance,
        reported_candidate: reportedValue, derived_candidate: derivedValue,
        warnings: [],
      };
    }
    return {
      status: "OK", value: reportedValue, provenance: reportedProvenance,
      reported_candidate: reportedValue, derived_candidate: null,
      warnings: [],
    };
  }
  ...
  if (derivedValue != null) {
    return {
      status: "OK", value: derivedValue, provenance: "DERIVED",
      reported_candidate: null, derived_candidate: derivedValue,
      warnings: [],
    };
  }

  return {
    status: "DATA_UNAVAILABLE", value: null, provenance: null,
    reported_candidate: null, derived_candidate: null, warnings: [],
  };
}

// ==================================================
// ==================================================
// valuateAccountEquity -- capa de valuacion, SEPARADA de computeNetWorth().
// 100% pura, cero fetch. El precio se recibe ya resuelto (por quien sea
// que hoy obtiene precios de mercado -- /api/market-data en el frontend,
// o su equivalente server-side).
// ==================================================
// account_snapshot_balances -> equity_value en unidades del asset
// valuation layer (esta funcion) -> convierte a USD
// computeNetWorth() -> recibe solo valores ya en USD, sin cambios a su contrato
export function valuateAccountEquity({ account_id, asset_id, equity_value, price_usd_per_unit }) {
  if (equity_value == null) {
    return { account_id, asset_id, equity_value: null, price_usd_per_unit: price_usd_per_unit ?? null, value_usd: null, status: "DATA_UNAVAILABLE" };
  }
  if (price_usd_per_unit == null) {
    return { account_id, asset_id, equity_value, price_usd_per_unit: null, value_usd: null, status: "PRICE_UNAVAILABLE" };
  }
  return { account_id, asset_id, equity_value, price_usd_per_unit, value_usd: equity_value * price_usd_per_unit, status: "OK" };
}

// ==================================================
// selectLatestConfirmedSnapshot -- PURA. Dado un conjunto de snapshots
// (con account_id/observed_at/id/source_import_id) y el set de
// source_import_id ya confirmados, elige el mas reciente por
// observed_at (desempate por id descendente) para una cuenta dada.
// Nunca suma snapshots historicos -- siempre exactamente uno o ninguno.
// ==================================================
export function selectLatestConfirmedSnapshot(snapshots, confirmedImportIds, groupKeyValue, groupKeyField = "account_id") {
  const confirmedSet = confirmedImportIds instanceof Set ? confirmedImportIds : new Set(confirmedImportIds);
  const candidates = (snapshots || []).filter((s) => s[groupKeyField] === groupKeyValue && confirmedSet.has(s.source_import_id));
...
// ==================================================
// selectLatestPositionSnapshot -- PURA. Igual que arriba pero para
// derivative_position_snapshots, que son append-only por diseño (todo
// lo que existe en esa tabla ya es "confirmado" por construccion --
// solo el RPC de confirm los inserta). No requiere set de confirmados.
// ==================================================
export function selectLatestPositionSnapshot(snapshots, derivativePositionId) {
  const candidates = (snapshots || []).filter((s) => s.derivative_position_id === derivativePositionId);
  if (candidates.length === 0) return null;
  return candidates.slice().sort((a, b) => {
    if (a.observed_at !== b.observed_at) return a.observed_at < b.observed_at ? 1 : -1;
    return b.id - a.id;
...
export function routeDerivativeSnapshot(normalizedFacts, reconciliationResult) {
  ...
  // MATCH_EXISTING_MEDIUM se suma aqui (politica final): confianza media
  // no puede decidir identidad financiera por si sola -- va a REVIEW,
  // igual que AMBIGUOUS/INSUFFICIENT_DATA. Nunca propone CREATE.
  if (decision === "POSITION_IDENTITY_AMBIGUOUS" || decision === "INSUFFICIENT_DATA" || decision === "MATCH_EXISTING_MEDIUM") {
    return [{
  ...
  proposals.push({
    ...
    fields: {
      // position_quantity: cantidad del SUBYACENTE -- puede ser null
      // (ej. USD-M cuando solo se reporta notional, nunca cantidad de BTC).
      // Nunca se rellena artificialmente con notional ni con 0.
      ...
      // notional: exposicion en moneda de cotizacion -- concepto SEPARADO
      // de position_quantity, nunca se mezclan. Antes de este fix, estos
      // 3 campos no se incluian aqui en absoluto -- gap real encontrado
      // durante esta ronda, no solo el de MEDIUM.
...
// ==================================================
// evaluateDateUpdateEligibility -- Confirm, PENDING->EXECUTED
// ==================================================
// Decide si la fecha de una transaccion PENDING puede actualizarse con
// la nueva evidencia. Regla: confidence estrictamente superior Y
// determinable. Si la confidence original no se puede determinar
// (transaccion no vino de Smart Import, o el import original nunca
// guardo confidence de fecha), se trata como UNKNOWN -- nunca se
// actualiza automaticamente, y si las fechas difieren, se manda a
// REVIEW en vez de proceder.
const CONFIDENCE_RANK = { HIGH: 3, MEDIUM: 2, LOW: 1 };

export function evaluateDateUpdateEligibility({ originalDate, originalConfidence, newDate, newConfidence }) {
  if (originalDate === newDate) {
    return { action: "NO_CHANGE", reason: "dates_identical" };
  }
  const originalRank = CONFIDENCE_RANK[originalConfidence] ?? 0; // 0 = UNKNOWN, nunca se asume LOW
  if (originalRank === 0) {
    return { action: "REQUIRES_REVIEW", reason: "ORIGINAL_DATE_CONFIDENCE_UNKNOWN" };
  }
  const newRank = CONFIDENCE_RANK[newConfidence] ?? 0;
  if (newRank > originalRank) {
    return { action: "UPDATE_ALLOWED", reason: `confidence_improved_${originalConfidence}_to_${newConfidence}` };
  }
  return { action: "REQUIRES_REVIEW", reason: "NEW_DATE_CONFIDENCE_NOT_STRONGER" };
}

// ==================================================
// evaluateIdentityFieldConsistency -- Confirm, PENDING->EXECUTED
// ==================================================
// quantity/amount son IMMUTABLE_IDENTITY_FIELD -- nunca se sobreescriben
// con el UPDATE. Esta funcion es solo un portal de seguridad: si la
// nueva evidencia difiere MATERIALMENTE de lo que la transaccion PENDING
// ya tenia, no es seguro asumir que es la misma operacion -- se manda a
// REVIEW en vez de fusionar en silencio. Diferencias pequeñas (ej.
// 8.786913824 vs 8.791672527, variacion real de fill vs estimado) quedan
// dentro de tolerancia: se procede, y el valor ORIGINAL de la
// transaccion PENDING es el que se conserva siempre -- este chequeo
// nunca decide escribir el valor nuevo, solo si es seguro proceder.
//
// FORMULA: diferencia porcentual relativa al valor ORIGINAL --
//   pct_diff = |original - nuevo| / |original| * 100
// (nunca absoluta -- una diferencia de 0.5 unidades es material en una
// compra de 1 unidad, irrelevante en una de 10,000).
//
// THRESHOLDS explicitos, nombrados, exportados -- nunca literales sueltos
// dentro de la funcion:
export const MATERIAL_DIFF_QUANTITY_PCT = 2; // % maximo tolerado en quantity
export const MATERIAL_DIFF_AMOUNT_PCT = 2;   // % maximo tolerado en amount (comparando magnitudes absolutas)

function pctDiff(original, updated) {
  if (original === 0) return updated === 0 ? 0 : 100;
  return (Math.abs(original - updated) / Math.abs(original)) * 100;
}

export function evaluateIdentityFieldConsistency({ originalQuantity, newQuantity, originalAmount, newAmount }) {
  // NULL en cualquiera de los 4 valores: no se puede verificar consistencia
  // de forma segura -- REQUIRES_REVIEW, nunca se asume que "sin dato" es
  // lo mismo que "coincide".
  if (originalQuantity == null || newQuantity == null || originalAmount == null || newAmount == null) {
    return { action: "REQUIRES_REVIEW", reason: "MISSING_VALUE_FOR_CONSISTENCY_CHECK", quantityDiffPct: null, amountDiffPct: null };
  }

  const quantityDiffPct = pctDiff(originalQuantity, newQuantity);
  const amountDiffPct = pctDiff(Math.abs(originalAmount), Math.abs(newAmount));

  if (quantityDiffPct > MATERIAL_DIFF_QUANTITY_PCT || amountDiffPct > MATERIAL_DIFF_AMOUNT_PCT) {
    return { action: "REQUIRES_REVIEW", reason: "IDENTITY_FIELD_MATERIAL_DIFFERENCE", quantityDiffPct, amountDiffPct };
  }
  return { action: "PROCEED", reason: "within_tolerance", quantityDiffPct, amountDiffPct };
}
