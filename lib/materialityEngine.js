...
export function computeStrategicRelevance(facts, positionContext) {
  ...
  // Sprint P3.1B.1: bug de idioma corregido -- antes comparaba texto
  // crudo de tema (español) contra headline (ingles, via Finnhub) con
  // substring directo, nunca hacia match real. Ahora ambos lados se
  // normalizan PRIMERO a tags canonicos independientes de idioma
  // (lib/canonicalThemeTags.js) y se comparan como sets, nunca como
  // texto -- nunca traduccion automatica.
  const positionTags = tagsFromFields(tema, sector, strategicRole);
  const eventTags = tagsFromFields(facts.headline_raw);
  const overlapTags = positionTags.filter((t) => eventTags.includes(t));
  if (overlapTags.length > 0) { value += 20; evidence.push(`canonical_tag_overlap(${overlapTags.join(",")})`); }

  return {
    value: clamp0to100(value), method: "position_theme_and_headline_canonical_tag_overlap", evidence,
    position_tags: positionTags, event_tags: eventTags,
  };
}
