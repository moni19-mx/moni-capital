...
// v1.1.0 (Internal Opportunities Calibration, policy C+E): auditoria
// real sobre las 39 posiciones mostro HIGH_CONVICTION_LOW_WEIGHT
// disparando en 21/39 (54%) -- demasiado ancho para ser accionable,
// dominado por convicciones manuales nunca revisadas por el engine. Los
// thresholds NO cambiaron (aprobado explicitamente mantenerlos) -- lo
// que cambio es que la señal ahora se clasifica por la calidad de la
// evidencia detras del numero (ver computeConvictionWeightMismatch),
// nunca se descarta ningun caso.
export const INTERNAL_OPPORTUNITIES_POLICY_VERSION = "internal-opportunities-v1.1.0";
...
export function computeConvictionWeightMismatch({ ticker, convictionValue, convictionSource, weightPct, evidenceRefs }) {
  ...
  // Calibracion (policy C+E, v1.1.0): SOLO HIGH_CONVICTION_LOW_WEIGHT se
  // degrada a INFORMATIONAL cuando la convicción nunca paso por el
  // engine (`manual_thesis_unreviewed`) -- los thresholds no cambian,
  // el caso sigue calculandose y mostrandose, solo deja de contar como
  // ruido accionable. LOW_CONVICTION_HIGH_WEIGHT NUNCA se degrada
  // (cero casos reales hoy en las 39 posiciones -- auditado, no
  // ejercitado, logica intacta tal como se aprobo explicitamente).
  const isEngineSourced = convictionSource === "engine_accepted" || convictionSource === "engine_proposed_pending_review";
  const classification = (direction === "HIGH_CONVICTION_LOW_WEIGHT" && !isEngineSourced) ? "INFORMATIONAL" : "REVIEW_REQUIRED";
  ...
  return {
    ...
    explanation: direction === "HIGH_CONVICTION_LOW_WEIGHT"
      ? `Conviction ${convictionValue} (${convictionSource}) es alta pero el peso en portafolio es ${weightPct.toFixed(2)}% -- posible sizing insuficiente frente a la convicción real.${classification === "INFORMATIONAL" ? " Informativo: la convicción nunca pasó por el conviction engine, no cuenta para prioridad todavía." : ""}`
