...
// Micro-sprint P3.2.1, item 16 (regresion permanente del bug NEUTRAL
// real encontrado y corregido en vivo durante P3.2). Decision PURA:
// cuando el llamador (api/conviction-benchmark-temp.js) debe insertar
// una fila marcadora (dimension_id=null, effect="NEUTRAL") en
// thesis_dimension_effects para que fetchUnprocessedEvents() deje de
// re-seleccionar (y re-facturar al AI) un evento que YA fue evaluado
// con exito y no encontro efecto en ninguna dimension real.
//
// true solo cuando: (a) el AI genuinamente corrio y no produjo NINGUNA
// fila de efecto (ni siquiera un affected_dimensions vacio con
// component_deltas -- effectRowsCount ya refleja eso desde el
// llamador), Y (b) el resultado fue APPLIED o NEUTRAL -- nunca
// FAILED/NOT_ATTEMPTED, esos DEBEN reintentarse en la proxima corrida
// (un fallo de AI no es "evaluado sin efecto", es "no evaluado").
export function shouldInsertNeutralMarker({ effectRowsCount, aiStatus }) {
  return effectRowsCount === 0 && (aiStatus === "APPLIED" || aiStatus === "NEUTRAL");
}
