...
// Traduccion real->motor. La tabla `transactions` usa la convencion en
// espanol que ya existia antes de Smart Import ("compra"/"venta"); el
// motor de reconciliacion usa BUY/SELL. Se traduce aqui, en la frontera
// de I/O -- la funcion pura nunca necesita saber de este detalle del
// schema legacy.
const TYPE_TO_ENGINE_VOCAB = { compra: "BUY", venta: "SELL" };

function normalizeTransactionRow(row) {
  return {
    ...row,
    total: row.amount != null ? Math.abs(row.amount) : null,
    type: TYPE_TO_ENGINE_VOCAB[row.type] ?? row.type,
  };
}
...
export async function resolveDuplicateCheck(supabase, candidate) {
  ...
  const result = classifyDuplicate(candidate, normalized);

  // Metadata SEGURA de la transaccion coincidente, necesaria para que
  // buildProposedChange (pura) pueda decidir PENDING->EXECUTED. Se toma
  // del mismo fetch ya hecho -- no se hace una segunda consulta.
  let matchedTransaction = null;
  if (result.matchedTransactionId != null) {
    const row = normalized.find((t) => t.id === result.matchedTransactionId);
    if (row) {
      matchedTransaction = {
        id: row.id,
        status: row.status ?? null,
        provider_transaction_id: row.provider_transaction_id ?? null,
        transaction_fingerprint: row.transaction_fingerprint ?? null,
      };
    }
  }

  return { ...result, matchedTransaction };
}
