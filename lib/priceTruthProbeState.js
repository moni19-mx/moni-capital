...
// ================== Validacion ESTRICTA de contrato (item 2/3 de la revision) ==================
// HTTP 200 NUNCA es suficiente por si solo -- Vercel Authentication
// puede devolver 200 con una pagina HTML de auto-redirect en vez de
// dejar pasar la request al handler real. Estas funciones distinguen
// "el endpoint realmente corrio y devolvio su forma real" de
// "recibimos ALGO con status 200 que no es la respuesta de la app".

// Shape generico, reusado por market-data y futures-equity: nunca
// asume JSON valido solo porque el status es 2xx.
export function classifyJsonResponseShape({ status, contentType, rawText }) {
  if (!(status >= 200 && status < 300)) return "HTTP_NON_2XX";
  let parsed;
  try { parsed = JSON.parse(rawText); } catch { parsed = undefined; }
  if (parsed === undefined) {
    const isHtml = (contentType || "").includes("text/html") || /^\s*</.test(rawText || "");
    return isHtml ? "HTTP_200_HTML" : "HTTP_200_INVALID_JSON";
  }
  return "HTTP_200_VALID_JSON";
}

// Contrato real de api/futures-equity.js (verificado en el codigo,
// nunca inventado): siempre responde
// {total_value_usd, is_complete, accounts, positions, warnings} en un
// 200 real. total_value_usd arranca en `let totalValueUsd = 0` y solo
// se le suma -- nunca es null en una ejecucion real del handler.
// is_complete arranca en `let isComplete = true` -- siempre boolean,
// nunca null. Si cualquiera de los dos aparece null/de otro tipo, la
// respuesta NO vino de este handler tal cual esta hoy.
const FUTURES_EQUITY_EXPECTED_KEYS = ["total_value_usd", "is_complete", "accounts", "positions", "warnings"];

export function validateFuturesContract(json) {
  if (!json || typeof json !== "object") return { valid: false, missingKeys: FUTURES_EQUITY_EXPECTED_KEYS, reason: "not_an_object" };
  const missingKeys = FUTURES_EQUITY_EXPECTED_KEYS.filter((k) => !(k in json));
  if (missingKeys.length > 0) return { valid: false, missingKeys, reason: "missing_keys" };
  if (typeof json.total_value_usd !== "number") return { valid: false, missingKeys: [], reason: "total_value_usd_not_number" };
  if (typeof json.is_complete !== "boolean") return { valid: false, missingKeys: [], reason: "is_complete_not_boolean" };
  return { valid: true, missingKeys: [], reason: null };
}

// Contrato real de api/market-data.js (verificado en el codigo):
// siempre responde {data, errors, updatedAt, provider_health,
// finnhub_status, coingecko_status} en un 200 real -- data es un
// objeto (posiblemente vacio), errors siempre un array.
const MARKET_DATA_EXPECTED_KEYS = ["data", "errors"];

export function validateMarketDataContract(json) {
  if (!json || typeof json !== "object") return { valid: false, missingKeys: MARKET_DATA_EXPECTED_KEYS, reason: "not_an_object" };
  const missingKeys = MARKET_DATA_EXPECTED_KEYS.filter((k) => !(k in json));
  if (missingKeys.length > 0) return { valid: false, missingKeys, reason: "missing_keys" };
  if (typeof json.data !== "object" || json.data === null) return { valid: false, missingKeys: [], reason: "data_not_object" };
  if (!Array.isArray(json.errors)) return { valid: false, missingKeys: [], reason: "errors_not_array" };
  return { valid: true, missingKeys: [], reason: null };
}

// Resultado final por endpoint -- VALID_ENDPOINT_RESPONSE solo si el
// shape es HTTP_200_VALID_JSON Y el contrato real del codigo se
// cumple. Cualquier otro caso (HTML, JSON invalido, JSON valido pero
// con shape distinto al contrato real, no-2xx) se clasifica igual:
// PLATFORM_INTERCEPT_OR_INVALID_RESPONSE -- nunca se cuenta como una
// falla de Price Truth, porque el codigo de Price Truth nunca llego a
// correr.
export function classifyEndpointDiagnosticResult({ status, contentType, rawText, validateContract }) {
  const shape = classifyJsonResponseShape({ status, contentType, rawText });
  if (shape !== "HTTP_200_VALID_JSON") return { result: "PLATFORM_INTERCEPT_OR_INVALID_RESPONSE", shape, contract: null, topLevelKeys: [] };
  let json = null;
  try { json = JSON.parse(rawText); } catch { /* inalcanzable, shape ya lo confirmo parseable */ }
  const contract = validateContract(json);
  return {
    result: contract.valid ? "VALID_ENDPOINT_RESPONSE" : "PLATFORM_INTERCEPT_OR_INVALID_RESPONSE",
    shape, contract, topLevelKeys: json && typeof json === "object" ? Object.keys(json) : [],
  };
}

// Verdict del modo diagnostico -- vocabulario DISTINTO de PASS/FAIL
// (nunca se confunde con una certificacion de Price Truth). OK
// significa "el harness produce resultados VALIDOS interpretables por
// contrato real, no solo HTTP 2xx" -- eso solo lo dice la corrida de
// 16 iteraciones. Cada item de marketDataResults/futuresResults debe
// traer `endpointResult` ("VALID_ENDPOINT_RESPONSE" o
// "PLATFORM_INTERCEPT_OR_INVALID_RESPONSE"), no solo status.
export function buildDiagnosticVerdict({ marketDataResults, futuresResults }) {
  const marketDataAllValid = marketDataResults.every((r) => r.endpointResult === "VALID_ENDPOINT_RESPONSE");
  const futuresAllValid = futuresResults.every((r) => r.endpointResult === "VALID_ENDPOINT_RESPONSE");
  return marketDataAllValid && futuresAllValid ? "DIAGNOSTIC_OK" : "DIAGNOSTIC_BLOCKED";
}
...
// ================== Universo dinamico de certificacion ==================
// MODE=certification (a diferencia de MODE=diagnostic, que siempre usa
// el subset fijo de 9 tickers) puede recibir un universo externo --
// positions UNION watchlist deduplicado + USDT, resuelto contra la
// tabla `assets` real ANTES de correr (ver scripts/priceTruthProbe.mjs).
// Esta funcion es pura: no llama Supabase ni la red, solo valida/normaliza
// la lista ya resuelta que le pasan, y garantiza que el subset trackeado
// explicito siga presente -- nunca inventa un ticker ni le asigna type
// por default.
export const TRACKED_SUBSET_TICKERS = ["ETH", "BTC", "LINK", "SOL", "USDT", "AMZN", "META", "NVDA", "QCOM"];

export function resolveCertificationUniverse(entries, fallback) {
  if (!entries) return { universe: fallback, source: "fallback_fixed_list", missingTrackedSubset: [] };
  if (!Array.isArray(entries) || entries.length === 0) {
    throw new Error("CERT_TICKERS debe ser un array no vacio de {ticker,type}");
  }
  const seen = new Set();
  const deduped = [];
  for (const entry of entries) {
    if (!entry || typeof entry.ticker !== "string" || entry.ticker.length === 0) {
      throw new Error(`CERT_TICKERS contiene una entrada invalida (ticker faltante): ${JSON.stringify(entry)}`);
    }
    if (entry.type !== "crypto" && entry.type !== "stock") {
      throw new Error(`CERT_TICKERS contiene una entrada invalida (type debe ser "crypto" o "stock", nunca adivinado): ${JSON.stringify(entry)}`);
    }
    if (seen.has(entry.ticker)) continue; // deduplicado, nunca cuenta dos veces el mismo ticker
    seen.add(entry.ticker);
    deduped.push({ ticker: entry.ticker, type: entry.type });
  }
  const dedupedTickers = new Set(deduped.map((e) => e.ticker));
  const missingTrackedSubset = TRACKED_SUBSET_TICKERS.filter((t) => !dedupedTickers.has(t));
  return { universe: deduped, source: "dynamic", missingTrackedSubset };
}
