...
// Micro-sprint P3.2.1 -- bugfix real encontrado en la corrida en vivo
// de validacion (QCOM/AMD/ANET/VRT): la condicion original en
// api/conviction-benchmark-temp.js exigia events.length>0 para crear
// una decision, asumiendo que solo un evento nuevo podia disparar una
// revision. Con fundamentales deterministicos (items 1-9) una corrida
// SIN eventos nuevos puede producir un delta real y justificado (p.ej.
// QCOM 3->4 solo por evidencia fundamental, coverage 52%, confidence
// 83%) que review.requires_user_review y recommendation_status ya
// marcan como legitimo -- exigir eventos ademas de eso enmascaraba
// silenciosamente una recomendacion real (decision_id quedaba null en
// los 4 activos reales de esa corrida). Extraida a funcion pura para
// que este trigger quede protegido por un test explicito.
export function shouldCreateReviewDecision({ hasNewEvidenceThisRun, requiresUserReview, recommendationStatus }) {
  return hasNewEvidenceThisRun === true && requiresUserReview === true && recommendationStatus === "PROPOSED_CHANGE";
}
