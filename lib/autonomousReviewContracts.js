...
// Fase B (Phase B expuso este hueco real de Fase A, ver revision):
// un REVIEW_RESPONSE puede ser schema-valido y AUN ASI ser
// semanticamente incoherente consigo mismo -- sin necesitar un packet
// para detectarlo. Dos casos explicitos exigidos:
//   - verdict=PASS con blocking_findings o required_changes no vacios
//     (un PASS real no deja hallazgos bloqueantes pendientes)
//   - verdict=BLOCKED_HUMAN con human_action=null (BLOCKED_HUMAN sin
//     decir que accion humana se necesita no es accionable)
// Complementa (no reemplaza) reviewerVerdictIsInternallyConsistent,
// que sigue siendo la unica que compara contra el packet.
export function reviewerResponseIsSelfConsistent(response) {
  if (response.verdict === "PASS") {
    if ((response.blocking_findings || []).length > 0) {
      return { consistent: false, reason: "verdict_PASS_but_blocking_findings_not_empty" };
    }
    if ((response.required_changes || []).length > 0) {
      return { consistent: false, reason: "verdict_PASS_but_required_changes_not_empty" };
    }
    if (response.human_action !== null) {
      return { consistent: false, reason: "verdict_PASS_but_human_action_not_null" };
    }
  }
  if (response.verdict === "BLOCKED_HUMAN" && response.human_action === null) {
    return { consistent: false, reason: "verdict_BLOCKED_HUMAN_but_human_action_is_null" };
  }
  return { consistent: true, reason: null };
}
