...
// ================== FINANCIAL CORRECTNESS (bug real: PnL contaminado) ==================
// Bug real encontrado en produccion: computeTotalGain(patrimonio, invested)
// se llamaba con `patrimonio` = TOTAL_NET_WORTH (incluye Futures Equity,
// ver computePatrimonio) contra `invested` = TRADITIONAL_INVESTED_CAPITAL
// (nunca incluyo Futures -- Futures no tiene cost_basis en `positions`).
// Resultado: el 100% de Futures Equity se contaba como ganancia, sin
// importar cuanto capital real entro a esas cuentas. El fix es de UN
// argumento -- computeTotalGain(a,b)=a-b siempre fue correcta y generica,
// el problema era el llamador. Contrato explicito, versionado aqui para
// que no se reintroduzca la confusion:
//
//   TRADITIONAL_PNL        = computeTotalGain(patrimonioBase, invested)
//   TRADITIONAL_RETURN_PCT = TRADITIONAL_PNL / invested
//   TOTAL_NET_WORTH        = computePatrimonio(patrimonioBase, futuresEquityUsd)
//   FUTURES_TOTAL_PNL      = FUTURES_TOTAL_PNL_STATUS (nunca un numero --
//     no conocemos de forma defendible el capital historico aportado a
//     USD-M/COIN-M: contributions/withdrawals/transfers/cost basis de
//     Futures no existen todavia)
//   TOTAL_PNL              = TOTAL_PNL_STATUS (mismo motivo -- NUNCA
//     TOTAL_NET_WORTH - TRADITIONAL_INVESTED_CAPITAL, ese es exactamente
//     el bug real)
export const FUTURES_TOTAL_PNL_STATUS = "DATA_UNAVAILABLE";
export const TOTAL_PNL_STATUS = "DATA_UNAVAILABLE";

// Reconstruye TRADITIONAL_MARKET_VALUE desde las columnas YA
// persistidas de una fila real de `snapshots` -- nunca lee
// `snapshotRow.patrimonio` (contaminado con Futures Equity desde que
// Futures se confirmo por primera vez, 2026-09-08). Determinístico, sin
// migracion de schema: stocks_value/crypto_value/cash_value existen en
// cada fila desde el primer snapshot real, incluidas las 2 filas
// contaminadas -- se pueden reconstruir sin reescribir ni una fila.
export function reconstructTraditionalMarketValue(snapshotRow) {
  const stocks = Number(snapshotRow?.stocks_value) || 0;
  const crypto = Number(snapshotRow?.crypto_value) || 0;
  const cash = Number(snapshotRow?.cash_value) || 0;
