...
//
// Architecture v1.1.1 (correccion critica post-testing):
// - Nivel A (precio, cambio diario) usa fetchMarketForAI: vivo primero,
//   cache corta de 60s solo si el proveedor en vivo falla.
// - Consultas de precio por posicion corren con concurrencia limitada
//   (5-8 a la vez), nunca secuencial ticker-por-ticker ni todas a la vez.
// - Cuando faltan precios de ALGUNAS posiciones (no todas), el agregado
//   se marca status:"partial" con completeness{coveragePct,
//   missingTickers} -- NUNCA se presenta un total incompleto como si
//   fuera completo.
// - Formulas compartidas (score, salud del sistema, desviaciones de
//   rebalanceo) vienen de financialMath.js, la MISMA libreria que usa
//   src/App.jsx -- una sola fuente de verdad, cero drift matematico.
//
// Fase 0 / Asset Master (revision critica Paso 6): el join entre
// positions y thesis ahora usa asset_id como llave, no ticker. Con el
// LEGACY_ASSET_MISMATCH confirmado en cero (Stage A), este cambio es
// equivalente al comportamiento anterior -- cada asset_id resuelve al
// mismo ticker que antes. Es el unico cambio de este archivo respecto a
// la version previa.

import { fetchMarketForAI, mapWithConcurrency } from "./aiPriceCache.js";
import { ok, partial, notFound, toolError } from "./toolResult.js";
import {
...
async function enrichPositions(supabase, FINNHUB_KEY) {
  ...
  const thesisByAssetId = {};
  (thesisRows || []).forEach((t) => { thesisByAssetId[t.asset_id] = t; });
  ...
  const marketResults = await mapWithConcurrency(nonCash, CONCURRENCY, async (p) => {
    const { market } = await fetchMarketForAI(supabase, p.ticker, p.type, p.coingecko_id, FINNHUB_KEY);
  ...
  marketResults.forEach((r) => {
    if (!r) return;
    const { position: p, market } = r;
    if (!market) missingTickers.push(p.ticker);
    const value = market ? Number(p.shares) * market.price : null;
    enriched.push({ ...p, market, value, thesis: thesisByAssetId[p.asset_id] || null });
  });

  const totalTickers = nonCash.length;
  const coveragePct = totalTickers ? round2(((totalTickers - missingTickers.length) / totalTickers) * 100) : 100;

  return { enriched, cashValue, missingTickers, coveragePct, totalTickers };
}

export async function get_portfolio_summary(supabase, FINNHUB_KEY) {
  const { enriched, cashValue, missingTickers, coveragePct, totalTickers } = await enrichPositions(supabase, FINNHUB_KEY);
  if (totalTickers === 0 && cashValue === 0) return notFound("supabase");

  const withValue = enriched.filter((p) => p.value != null);
  if (totalTickers > 0 && missingTickers.length === totalTickers) {
    return toolError("provider_timeout", "finnhub/coingecko");
  }
  ...
  const data = {
    ...
    posiciones_sin_precio_ahora: missingTickers,
  ...
  if (missingTickers.length > 0) {
    return partial(data, { coveragePct, missingTickers }, "computed");
  }
  return ok(data, "computed");
}
...
export async function get_position(supabase, FINNHUB_KEY, ticker) {
  ...
  if (pos.type !== "cash") {
    const r = await fetchMarketForAI(supabase, pos.ticker, pos.type, pos.coingecko_id, FINNHUB_KEY);
    market = r.market;
    if (r.failed) return toolError("provider_timeout", pos.type === "stock" ? "finnhub" : "coingecko");
    if (market) source = r.source;
  }
...
export async function get_opportunities(supabase, FINNHUB_KEY) {
  const { enriched, missingTickers, coveragePct, totalTickers } = await enrichPositions(supabase, FINNHUB_KEY);
  ...
  for (const w of watchlist || []) {
    const { market } = await fetchMarketForAI(supabase, w.ticker, w.type, w.coingecko_id, FINNHUB_KEY);
    if (!market) { watchlistMissing.push(w.ticker); continue; }
  ...
  const allMissing = [...missingTickers, ...watchlistMissing];

  if (result.length === 0) {
    if (totalTickers > 0 && missingTickers.length === totalTickers) return toolError("provider_timeout", "finnhub/coingecko");
    return notFound("computed");
  }
  if (allMissing.length > 0) {
    return partial(result, { coveragePct, missingTickers: allMissing }, "computed");
  }
...
export async function get_strategy_status(supabase, FINNHUB_KEY) {
  ...
  const data = { badges, peso_posicion_1_pct: round2(top1Pct * 100) };
  if (missingTickers.length > 0) return partial(data, { coveragePct, missingTickers }, "computed");
  return ok(data, "computed");
}
...
export async function get_system_health(supabase, FINNHUB_KEY) {
  ...
  const data = { puntaje: health.score, factores: health.factors };
  if (missingTickers.length > 0) return partial(data, { coveragePct, missingTickers }, "computed");
  return ok(data, "computed");
}
...
export const TOOL_DEFINITIONS = [
  ...
  { name: "get_position", description: "Detalle de una posición específica: acciones, costo base, precio actual, ganancia y convicción declarada.", input_schema: { type: "object", properties: { ticker: { type: "string" } } } },
  { name: "get_thesis", description: "La Investment Thesis de un ticker: por qué se compró, qué tiene de especial, Exit Thesis (criterio de salida), horizonte y riesgos.", input_schema: { type: "object", properties: { ticker: { type: "string" } } } },
