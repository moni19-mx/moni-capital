...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (resource === "generate_insight") {
      ...
      // Cambio de comportamiento deliberado: si la generacion fallo (error
      // tecnico real, no incertidumbre de datos), NO se guarda un insight
      // en ai_insights. Antes esto se guardaba igual con contenido vacio y
      // una confidence numerica, lo cual el usuario ve como un Daily Brief
      // valido de baja confianza -- y no lo es, es un fallo tecnico que
      // deberia poder reintentarse, no quedar cacheado bajo el hash del dia.
      if (r.error) {
      ...
      try {
        ...
        // Si el modelo respondio (no hubo error tecnico) pero el texto no
        // es JSON valido, sigue siendo un problema real de formato -- pero
        // ahora es la unica situacion legitima para guardar con
        // estado_general crudo, porque aqui SI sabemos que el modelo
        // termino de buena fe (stopReason end_turn con contenido real).
